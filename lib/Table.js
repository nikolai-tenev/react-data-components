...
var getCellValue = function getCellValue(_ref, row) {
    ...
    //Use the default content as cell value if the actual value isn't provided.
    var cellValue = !isEmpty(prop) && isEmpty(row[prop]) ? defaultContent : row[prop];

    //If render function is provided - call it with cell value, otherwise - just return.
    return render ? render(cellValue, row) : cellValue;
};
